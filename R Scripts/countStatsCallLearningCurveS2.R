...
fileIn <- '/Users/frederictheunissen/Code/zebra-finch-memory-lesions/data/behavior/fig5S2CallCount.csv'
...
# Learning curve models for song
k_model <- glmer('cbind(ints, tots-ints) ~ k*Rewarded + ((1 + k*Rewarded)|Subject)', data = learning_curve_counts, subset = (learning_curve_counts$CallType == ctype), family = binomial)
full_model <- glmer('cbind(ints, tots-ints) ~ Treatment*k*Rewarded + ((1 + k*Rewarded)|Subject)', data = learning_curve_counts, subset = (learning_curve_counts$CallType == ctype), family = binomial)
...
full_modelCtrlNCM <- glmer('cbind(ints, tots-ints) ~ Treatment*k*Rewarded + ((1 + k*Rewarded)|Subject)', 
                           ...
                           subset = ((learning_curve_counts$Treatment != 'HVC') & (learning_curve_counts$CallType == ctype)))
...
# HVC vs NCM
full_modelHVCNCM <- glmer('cbind(ints, tots-ints) ~ Treatment*k*Rewarded + ((1 + k*Rewarded)|Subject)', 
                           data = learning_curve_counts, family = binomial,
                           subset = ((learning_curve_counts$Treatment != 'CTRL')&(learning_curve_counts$CallType == ctype)))
...
full_modelCtrlHVC <- glmer('cbind(ints, tots-ints) ~ Treatment*k*Rewarded + ((1 + k*Rewarded)|Subject)', 
                          data = learning_curve_counts, family = binomial,
                          subset = ((learning_curve_counts$Treatment != 'NCM')&(learning_curve_counts$CallType == ctype)))
...
k_model <- glmer('cbind(ints, tots-ints) ~ k*Rewarded + ((1 + k*Rewarded)|Subject)', data = learning_curve_counts, family = binomial, subset = (learning_curve_counts$CallType == ctype))
full_model <- glmer('cbind(ints, tots-ints) ~ Treats*k*Rewarded + ((1 + k*Rewarded)|Subject)', data = learning_curve_counts, family = binomial, subset = (learning_curve_counts$CallType == ctype))
