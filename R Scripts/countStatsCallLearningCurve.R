...
fileIn <- '/Users/frederictheunissen/Code/zebra-finch-memory-lesions/data/behavior/fig5CallCount.csv'
...
k_model <- glmer('cbind(ints, tots-ints) ~ k*Rewarded + ((1 + Rewarded)|Subject)', data = learning_curve_counts, subset = (learning_curve_counts$CallType == ctype), family = binomial)
full_model <- glmer('cbind(ints, tots-ints) ~ Treatment*k*Rewarded + ((1 + Rewarded)|Subject)', data = learning_curve_counts, subset = (learning_curve_counts$CallType == ctype), family = binomial)
...
# HVC vs NCM
full_modelHVCNCM <- glmer('cbind(ints, tots-ints) ~ Treatment*k*Rewarded + ((1 + Rewarded)|Subject)', 
                           data = learning_curve_counts, family = binomial,
...
k_model <- glmer('cbind(ints, tots-ints) ~ k*Rewarded + ((1 + Rewarded)|Subject)', data = learning_curve_counts, family = binomial, subset = (learning_curve_counts$CallType == ctype))
full_model <- glmer('cbind(ints, tots-ints) ~ Treats*k*Rewarded + ((1 + Rewarded)|Subject)', data = learning_curve_counts, family = binomial, subset = (learning_curve_counts$CallType == ctype))
